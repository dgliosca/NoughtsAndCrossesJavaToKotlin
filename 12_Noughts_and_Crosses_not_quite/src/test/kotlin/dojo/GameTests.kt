...
class GameTests {
    ...
    @Test
    fun `cross makes the first move`() {
        val game = Game()
    ...
    fun `player makes invalid move`() {
        val game = Game()
        assertThat({ game.makeMove(Move(100, 100), X) }, throws(has(Game.InvalidMove::message, equalTo("Invalid move"))))
    }
    ...
    fun `player try to make a move in a location already taken`() {
        ...
        assertThat({ firstMoveGame.makeMove(Move(0, 0), game.nextCellToPlace()) }, throws(has(Game.InvalidMove::message, equalTo("Invalid move"))))
    }
